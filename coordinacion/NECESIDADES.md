# NECESIDADES — Nastia → agentic-install

Canal de peticiones entre proyectos. **Nastia** (SO web en el navegador,
`~/work2026/nastia`, producción en nastia.utopiaia.com) pide aquí; el agente de
...
`PENDIENTE → EN CURSO → SERVIDO → VALIDADO` (Nastia pone VALIDADO tras probarlo
...
Contexto técnico para servir modelos a Nastia:
...
- Los pesos autoalojados van en `/Users/kikocisneros/work2026/nastia/web/models/`
...
Queremos el primer **modelo propio** de Nastia en el navegador. Base
...
- **Qué**: fine-tune (LoRA fusionado) sobre gemma-4-E2B-it con el protocolo de
  tool-calls de Nastia (ver arriba) + español conciso, exportado a **`.litertlm`
  CON artefactos WebGPU** (gotcha conocido: LiteRT-LM issue #2322 — un
  `.litertlm` sin artefactos WebGPU no compila en navegador).
- **Entrega**: `web/models/nastia-e2b-v1.litertlm` (o URL HF tipo
  `KikoCis/nastia-e2b-litert-lm`). Nastia solo tiene que cambiar `MODEL_URL` en
...
Un 0.5B **entrenado para el protocolo de Nastia** rendiría mucho más que uno
genérico ×4 su tamaño.

- **Qué**: SFT sobre `Qwen/Qwen2.5-0.5B-Instruct` (o similar ≤1B) con dataset
  sintético del protocolo Nastia: pares user→tool-call para las 14 herramientas,
...
- **Entrega**: export ONNX con optimum + cuantización **q4** (¡no q4f16!) en
  `web/models/nastia-0.5b-v1/` con estructura HF completa
  (`config.json`, `tokenizer.json`, `onnx/model_q4.onnx`…). Nastia lo activa con
