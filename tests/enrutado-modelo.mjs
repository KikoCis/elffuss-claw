...
//   node tests/enrutado-modelo.mjs --ejemplos enteros   # solo cambia el catálogo
//
// Mismo prompt de sistema que la app —persona y reglas leídas de agent.js,
// ejemplos de tool-router.js—, temperatura 0 y semilla fija. Entre las dos
// condiciones cambia lo mismo que cambia en la app: el bloque de HERRAMIENTAS y
// los ejemplos, que viajan con su familia. Con `--ejemplos enteros` van los
// cinco ejemplos en las dos condiciones y lo único que cambia es el catálogo:
// sirve para separar qué efecto es de cada recorte. Cuando el recuperador duda
// y manda el catálogo entero, las dos condiciones son idénticas y se genera una
// sola vez.
//
...
const { crearRouter, bloqueEjemplos } = await import(path.join(WEB, 'tool-router.js'));
...
// El prompt de la app, con dos huecos: el bloque de herramientas y el de ejemplos.
function plantilla() {
  ...
  const fin = a.indexOf('${bloqueEjemplos(', ini);
  ...
  return (lineas, ejemplos) => antes + lineas.join('\n') + despues + ejemplos;
}
...
const promptEntero = prompt(lineas, bloqueEjemplos(null));

console.log(`casos: ${casos.length} (${path.basename(CASOS)}) · modelos: ${MODELOS.join(', ')} · ejemplos: ${EJEMPLOS}\n`);
const resultados = {};
...
for (const modelo of MODELOS) {
  ...
  for (const c of casos) {
    ...
    const recortado = r ? prompt(r.lineas, bloqueEjemplos(EJEMPLOS === 'enteros' ? null : r.lineas)) : null;
    ...
    filas.push({ c, ruta: r ? r.familias : null, fe, fr, oke: acierta(c, fe), okr: acierta(c, fr), pe: [res.entero.pe, res.recortado.pe], ped: [res.entero.ped, res.recortado.ped], te: res.entero.texto, tr: res.recortado.texto });
  ...
  // El prefill se da como RAZÓN entre condiciones, no en milisegundos: es la
  // cifra que importa y la única que no depende de la máquina donde se mide.
  const razon = media(filas, 0, 'ped') ? (media(filas, 1, 'ped') / media(filas, 0, 'ped')).toFixed(2) : '—';
  console.log(`  condición   acierta herramienta   charla sin llamar   prompt evaluado (tok)`);
  for (const [j, cond, k] of [[0, 'entero', 'oke'], [1, 'recortado', 'okr']]) {
    console.log(`  ${cond.padEnd(10)}  ${`${cuenta(herr, k)}/${herr.length}`.padStart(19)}   ${`${cuenta(charla, k)}/${charla.length}`.padStart(17)}   ${String(media(filas, j, 'pe')).padStart(21)}`);
  }
  console.log(`  prefill recortado / entero: ${razon}`);
